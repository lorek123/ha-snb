...
class TestStorzBickelDataUpdateCoordinator:
    ...
    async def test_connect_success(
        ...
        with (
            patch(BT_SCANNER, return_value=mock_ble_device),
            patch.object(
    ...
    async def test_connect_device_not_visible(self, hass: HomeAssistant, mock_entry):
        """Test connection when device is not visible to HA bluetooth scanner."""
        coordinator = StorzBickelDataUpdateCoordinator(hass, mock_entry)

        with (
            patch(BT_SCANNER, return_value=None),
    ...
    async def test_case_insensitive_address_matching(
        ...
        with (
            patch(BT_SCANNER, return_value=mock_ble_device) as mock_scanner,
            patch.object(
                StorzBickelClient,
                "connect_device",
                new_callable=AsyncMock,
    ...
    async def test_async_connect_noop_when_connected(
        ...
        coordinator.device = mock_device
        with patch.object(
            coordinator, "_async_connect", new_callable=AsyncMock
        ) as connect:
